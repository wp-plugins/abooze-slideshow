...
Easily upload images with links to display a nice slideshow on your website. To manage, Go to <strong>Media-> Aboozé Slideshow</strong>. To display the slideshow, add the shortcode: <pre><code> <?php if (function_exists('ab_show')){ ab_show(); }?></code></pre> in your template or simply adding the shortcode <pre><code>[ab_show]</code></pre> in the page from the admin panel.
...
Also please note that the images should be having the dimensions: <b>900x450 pixels</b> to upload. I'm working to allow users for choosing different dimensions.
